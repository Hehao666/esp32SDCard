...
void handleUserRequet() {         
  ...
  String contentType = "";
  path = URLDecode(path);
  if (strncmp(path.c_str(), "/audio/", 7) == 0)
  {
    handleAudioRequest(path);
  }
  else if (strncmp(path.c_str(), "/webgame/oldGame", 16) == 0) {
    ...
    if(FirstWebis==1)
      path = "/system/allFile.html";// 则将访问地址修改为/index.html便于SPIFFS访问
    else if(FirstWebis==2)
      path = "/system/upload.html";
    else if(FirstWebis==3)
      path = "/system/audio.html";
    else if(FirstWebis==4)
      path = "/system/webgame.html";
    else if(FirstWebis==5)
      path = "/system/video.html";
    else if(FirstWebis==6)
      path = "/system/clipboard.html";
    else if(FirstWebis==7)
      path = "/system/set.html";
    else if(FirstWebis==8)
      path = "/system/wifi.html";
