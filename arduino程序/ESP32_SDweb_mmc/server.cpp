...
int gameWidth=1120,gameHeight=840;
...
void configFile(){
  ...
  readConfig(SD_MMC);
  esp32_server.send(200, "text/html", "<html><body><script>window.location.href='/autowificonnect';</script></body></html>");
}
...
void nameText(){
  ...
  readConfig(SD_MMC);
  esp32_server.send(200, "text/html", "<html><body><script>window.location.href='/set.html';</script></body></html>");
}
...
void server_wifista(){
  ...
  {
    //sprintf(buffer, "第%d次连接wifi开始", j+1);  
  ...
  esp32_server.on("/autowificonnect",HTTP_GET, pageConfigWIFI);   //发送开机自连wifi网页
  ...
  esp32_server.on("/uploadfoldAll", HTTP_GET, quickAllFile);   //快速加载
  esp32_server.on("/whiteAll.html", HTTP_GET, listAllFile);   //列出文件总列表
  esp32_server.on("/uploadfold", HTTP_GET, quickfile);   //列出文件上传列表
  esp32_server.on("/white.html", HTTP_GET, listUploadFile);   //列出文件上传列表
  esp32_server.on("/lookthis", HTTP_GET, lookthisFile);   //查看文件
  esp32_server.on("/downloadUploadFile", HTTP_GET, downloadUploadFile);   //下载文件（可中文）
  esp32_server.on("/addFold", HTTP_GET, addFold);   //创建文件夹
  ...
  esp32_server.on("/deleteUploadAllFile", HTTP_GET, deleteUploadAllFile);   //删除文件
