...
void configAP()
{
  ...
  flag = writeFile(SD_MMC, "/config/password.txt", (char *)message.c_str()); // 将名称和密码写入SD卡
  if (flag)
  {
    ssid = ssid2;
    password = password2;
    channel = String2Char((char *)channel2.c_str());
    esp32_server.send(200, "text/html", "<!DOCTYPE html><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, minimum-scale=0.5, maximum-scale=2.0, user-scalable=yes\" /><title>配置成功</title></head><body><h1>配置成功</h1><br>请重启服务器</body></html>");
  }
  else
  {
    esp32_server.send(200, "text/html", "<!DOCTYPE html><head><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0, minimum-scale=0.5, maximum-scale=2.0, user-scalable=yes\" /><title>配置失败</title></head><body><h1>配置失败</h1></body></html>");
  }
}

void getWifi()
{
  Storage(); // 假设这是获取存储信息的函数，需要定义totalstorage和storage变量
  ...
  // 初始化JSON字符串
  strcpy(jsonBuffer, "{\"totalstorage\": \"");
  snprintf(tempBuffer, sizeof(tempBuffer), "%lld", totalstorage);
  strcat(jsonBuffer, tempBuffer);

  strcat(jsonBuffer, "\",\"storage\": \"");
  snprintf(tempBuffer, sizeof(tempBuffer), "%lld", storage);
  strcat(jsonBuffer, tempBuffer);

  strcat(jsonBuffer, "\",\"autoWifinum\": \"");
